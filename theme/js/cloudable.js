...
function loadImages(albumId, pageStart, perPage, filterOrderBy)
{
	...
	loadAjaxContent('#main-ajax-container', WEB_ROOT+"/ajax/load_file_manager.ajax.php", {nodeId: albumId, pageStart: pageStart, perPage: perPage, filterOrderBy: filterOrderBy}, successCallback);
}
...
function loadAlbumsByUserId(userId, pageStart, perPage, filterOrderBy)
{
	...
	loadAjaxContent('#profile-albums', WEB_ROOT+"/ajax/browse_public_albums.ajax.php", {userId: userId, pageStart: pageStart, perPage: perPage, filterOrderBy: filterOrderBy}, successCallback);
}
...
function loadBrowsePageAlbums(searchFilter, pageStart, perPage, filterOrderBy, advFilters)
{
	...
	loadAjaxContent('#browse-albums', WEB_ROOT+"/ajax/browse_public_albums.ajax.php", {searchFilter: currentSearchFilter, pageStart: pageStart, perPage: perPage, filterOrderBy: filterOrderBy, advFilters: advFilters}, successCallback, false);
}
...
function loadBrowsePageCategories()
{
	...
	loadAjaxContent('#browse-categories', WEB_ROOT+"/ajax/browse_public_categories.ajax.php", {}, successCallback, false);
}
...
function loadBrowsePageRecentImages(searchFilter, pageStart, perPage, filterOrderBy, advFilters)
{
	...
	loadAjaxContent('#browse-images', WEB_ROOT+"/ajax/load_file_manager.ajax.php", {searchFilter: currentSearchFilter, searchType: 'browserecent', pageStart: pageStart, perPage: perPage, filterOrderBy: filterOrderBy, advFilters: advFilters}, successCallback, false);
}
...
function viewFile(fileId)
{
	scrollTop();
	successCallback = function(data) {
		updatePageUrlBar(null, data.html, data.page_title);
		setupToolTips();
		showLayer('main-ajax-container');
		setupMobileImageSwipe();
	}
	loadAjaxContent('#main-ajax-container', WEB_ROOT+"/ajax/account_file_details.ajax.php", {f: fileId}, successCallback);
}
...
function updateRecentImagesSorting(key, label, ele)
{
	$('#filterOrderBy').val(key);
	$('#filterButton').html(label + ' <i class="entypo-arrow-combo"></i>');
	filterOrderBy = $('#filterOrderBy').val();
	loadBrowsePageRecentImages(currentSearchFilter, 1, 0, filterOrderBy);
}
...
function updateViewType()
{
	...
	$.ajax({
		dataType: "json",
		url: WEB_ROOT+"/ajax/update_view_type.ajax.php",
...
function updateSelectedFilesStatusText()
{
	...
	if (count == 1)
	{
		...
		updateStatusText(count + ' selected&nbsp;&nbsp;<a href="#" onClick="clearSelected(); return false;">(clear)</a>');
	}
	else if (count > 1)
	{
		totalFilesize = getSizeSelected();
...
function assignLiRightClick()
{
	...
	// clear selected files on left click on file manager window
	$(".gallery-env").click(function() {
		clearSelected();
	});
}

function setCurrentAlbumAsUploadFolder(albumId)
{
	if (typeof (setUploadFolderId) === 'function')
	{
		if((currentAlbumId == '-1') || ($.isNumeric(currentAlbumId)))
		{
			setUploadFolderId(currentAlbumId);
		}
	}
}
...
{
	showLoaderModal();
	jQuery('#editFileModal .modal-content').load(WEB_ROOT+"/ajax/account_edit_file.ajax.php", {fileId: fileId}, function() {
		hideLoaderModal();
...
function deleteFiles(fromFileDetails, callback)
{
	...
	if (countSelected() > 0)
	{
		text = 'Are you sure you want to remove the selected [[[TOTAL_FILES]]] file(s)?';
...
function deleteFilesConfirm(fromFileDetails, callback)
{
	...
	$.ajax({
		type: "POST",
		url: CORE_AJAX_WEB_ROOT + "/file_manage_bulk_delete.ajax.php",
		...
		success: function(json) {
			if (json.error == true)
			{
				addBulkError(json.msg + '<br/>');
			}
			else
			{
				addBulkSuccess(json.msg + '<br/>');
			}
	...
	if (isDesktopUser() == true)
	{
		...
		$('.fileIconLi:not(.fileDeletedLi)').draggable({
			...
			helper: function(event) {
				selectFile($(this).attr('fileId'), true);
				var ret = $(this).clone();
				var textStr = 'Moving ' + countSelected() + ' file(s)';
...
/*function moveFilesIntoFolder(newFolderId)
{
	...
	$.ajax({
		dataType: "json",
		url: WEB_ROOT+"/ajax/account_move_file_in_folder.ajax.php",
...
function updateAlbumCover(fileId)
{
	$.ajax({
		dataType: "json",
		url: WEB_ROOT+"/ajax/update_album_cover.ajax.php",
		data: {fileId: fileId},
		success: function(data) {
